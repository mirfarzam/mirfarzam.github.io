...
import urllib
import json
from blockchain.chain import Transaction, Block, Blockchain
...
def trans(cmd):
    """
        Do Transaction - Method to perform new transaction on blockchain.
    """
    txData = cmd.split("trans ")[-1]
    amount, sender, receiver = txData.split(" ")
    print "Doing transaction..."
    data = {
    'amount':amount,
    'sender':sender,
    ...
    try:
        coin.addTransaction(data)
        print("Transaction was successful : {} transfers {} NiceCoins to {}".format(sender, amount, receiver))
